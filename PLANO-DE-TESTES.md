# Plano de testes — demo de quinta (30/07)

Roteiro pra rodar antes da demo do Renan. Marque cada linha com ✅ (passou),
⚠️ (passou com ressalva — anote o quê) ou ❌ (falhou — anote o erro) conforme
formos testando. Não rode tudo de uma vez: valide um bloco, ajusta se precisar,
só então passa pro próximo.

Pré-requisito de todos os blocos: rodar o app localmente.
...
(Se `bun` não estiver disponível na sua máquina, `npm install && npx vite dev`
funciona igual — o projeto roda em Vite puro por baixo.)

App sobe em `http://localhost:3000`.

---

## 0. Baseline (antes de qualquer teste de fluxo)

- [ ] `bun dev` sobe sem erro no terminal
- [ ] Abrir `http://localhost:3000` — a tela carrega sem tela branca/erro no console
- [ ] Painel "Prontidão da config" mostra os LEDs — confirmar que HeyGen/Deepgram
      aparecem OK (chave vem do `.env`, não precisa digitar nada)
- [ ] `.env` local tem `HEYGEN_API_KEY`, `DEEPGRAM_API_KEY`, `RECALL_API_KEY`
      preenchidas (conferido: sim, as três já estão preenchidas)
- [ ] Se `APP_PASSWORD` estiver definida: tela de login aparece com a **logo
      oficial** (não mais o texto "G0"/wordmark solto) — ver item 9
- [ ] Se `APP_PASSWORD` estiver vazia: app abre direto, sem tela de login (login
      desativado por padrão — comportamento esperado)
...
- [ ] Legendas ao vivo aparecem na tela conforme a fala é transcrita
...
- [ ] Motor Deepgram (padrão): fala é transcrita corretamente
- [ ] Trocar para Web Speech API: idem
...
- [ ] Derrubar a rede momentaneamente durante o pré-aquecimento → sessão atual
...
- [ ] Bot entra na reunião com o nome "Avatar e Renan" na lista de participantes
...
- [ ] Nenhum bug conhecido sobrando ao final deste bloco (esse é o critério de
...
- [ ] Botão "📸 Capturar frame atual como poster" com o avatar conectado: captura
...
- [ ] ⚠️ O PNG atual é um placeholder genérico gerado por mim — trocar por um
